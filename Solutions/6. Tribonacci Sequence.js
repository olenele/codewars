...
that given a signature array/list, returns the first n elements - signature included of the 
so seeded sequence.
...
recommend to any math enthusiast and for showing me this mathematical curiosity too 
...
function tribonacci(signature,n){
  ...
  for (let i = 0; i < n; i++){
    if (i < 3) {
      solution = solution.concat(signature[i])
    } else {
      solution = solution.concat((solution[i-1] + solution[i-2] + solution[i-3]))
    }
  }
