// kata: https://www.codewars.com/kata/5427db696f30afd74b0006a3
// 5 kyu

/*
Description:
Task
Your task is to write a function for calculating the score of a 10 pin bowling game. The input for 
the function is a list of pins knocked down per roll for one player. Output is the player's total score.

Rules
General rules
Rules of bowling in a nutshell:

A game consists of 10 frames. In each frame the player rolls 1 or 2 balls, except for the 10th frame,
 where the player rolls 2 or 3 balls.
...
As the 10th frame is the last one, in case of spare or strike there will be no next balls for the 
bonus. To account for that:
...
var bowlingScore = function(rolls) {
  ...
  while (frame <= 10){
    if (rolls[i] === 10){ 
      score += rolls[i] + rolls[i+1] + rolls[i+2]; // strike, bonus ball, bonus ball
      i++; //next frame starts with next ball
    } else if (rolls[i] + rolls[i+1] === 10){ 
      score += rolls[i] + rolls[i+1] + rolls[i+2]; // first ball, spare ball, bonus ball
      i += 2; //skip second ball to get to the next frame
    } else {
      score += rolls[i] + rolls[i+1]; // no bonuses 
      i += 2; //skip second ball to get to the next frame
    }
    frame++;
  }
